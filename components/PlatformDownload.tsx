...
const PlatformDownload: React.FC<PlatformDownloadProps> = ({ onInstallClick }) => {
    ...
    return (
        <div className="bg-white dark:bg-brand-secondary p-4 sm:p-6 rounded-xl shadow-md dark:shadow-lg">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4 text-center">{t('dashboard.platformDownload.title')}</h3>
            <p className="text-center text-gray-500 dark:text-brand-muted mb-6">{t('dashboard.platformDownload.description')}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <button
                    onClick={() => onInstallClick('windows')}
                    className="flex items-center justify-center gap-3 w-full bg-gray-100 dark:bg-brand-primary p-4 rounded-lg font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors"
                >
