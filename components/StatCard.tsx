...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon, currency, isExpense = false }) => {
    ...
    return (
        <div className="bg-white dark:bg-brand-secondary p-4 sm:p-5 rounded-xl shadow-md dark:shadow-lg flex items-center gap-3 sm:gap-4 min-w-0 transition-transform transform sm:hover:scale-[1.02]">
            ...
            <div className="min-w-0">
                ...
                <p className={`text-lg sm:text-2xl font-bold ${valueColorClass} truncate`}>{formattedValue}</p>
            </div>
        </div>
    );
};
